...
OpticsExpert는 python으로 만든 빔 광학 계산 프로그램입니다. 오래전에 만들어진 빔 광학 프로그램들의 단점(Text User Interface)을 개선하여 편의성을 증가시키는것이 이 프로그램을 만든 이유입니다. 이 프로그램은 GPL 3.0 라이선스를 따른다. 따라서 교육용, 상업용을 포함한 모든 목적의 사용이 가능합니다.
...
1. Quadrupole Magnet (사극자석)
1. Dipole Magnet (이극자석)
1. Solenoid (솔레노이드)
1. Thin Lens(얇은 랜즈)
1. Collimators x/y
1. RF Gap (고주파 갭)
1. Monitor (=Drift Space)
...
상전도 사극 전자석 하나, 상전도 이극 전자석 하나를 등록해봅니다. 드리프트는 이미 생성되어 있으므로 생략합니다.
우선 + 버튼을 눌러 quadrupole 을 선택합니다.
